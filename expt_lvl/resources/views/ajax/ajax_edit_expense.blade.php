@isset($expenseData)
    <form action="javascript:void(0);">
        ...
        <div class="form-floating mb-3">
            <select class="form-select" id="update_expense_account" name="update_expense_account">
                ...
                @foreach ($bankAccountsName as $rsBankAccountsName)
                    <option value="{{ $rsBankAccountsName->id }}" {{ $rsBankAccountsName->id == $expenseData->account_id ? "Selected" : '' }} >{{ \App\ASPLibraries\CustomFunctions::decrypt( $rsBankAccountsName->account_name ) }}</option>
                @endforeach
            </select>
        ...
        <div class="form-floating mb-3">
            <input type="number" class="form-control" placeholder="Enter Expense Amount" id="update_expense_amount" name="update_expense_amount" value="{{ \App\ASPLibraries\CustomFunctions::decrypt( $expenseData->amount ) }}">
            <label class="focus-label">Enter Expense Amount</label>
        </div>
        <div class="form-floating mb-3">
            <textarea id="update_expense_description" name="update_expense_description" cols="30" rows="5" class="form-control h-auto" placeholder="Enter Expense Description">{{ \App\ASPLibraries\CustomFunctions::decrypt( $expenseData->description  ) }}</textarea>
